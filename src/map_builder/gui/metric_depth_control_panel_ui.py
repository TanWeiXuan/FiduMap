...
def build_metric_depth_controls(panel: Any, callbacks: dict[str, Callable[[], None]]) -> dict[str, ttk.Button]:
    ...
    settings = ttk.LabelFrame(frame, text="Model and prompt settings")
    settings.grid(row=4, column=0, sticky="ew", padx=6, pady=4)
    settings.columnconfigure(1, weight=1)
    ttk.Label(settings, text="Backend").grid(row=0, column=0, sticky="w", padx=6)
    ttk.Combobox(settings, textvariable=panel.backend_var, values=list(panel.BACKEND_LABELS), state="readonly").grid(row=0, column=1, sticky="ew", padx=6)
