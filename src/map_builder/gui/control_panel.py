...
class ControlPanel(ScrollableFrame):
    def __init__(
        ...
        **kwargs: object,
        ...
        ttk.Label(frame, textvariable=self.folder_var, wraplength=310).grid(row=1, column=0, sticky="ew", padx=8)
        ttk.Label(frame, textvariable=self.camera_var, wraplength=310).grid(row=2, column=0, sticky="ew", padx=8, pady=(0, 8))

        ttk.Button(frame, text="Refresh / Re-index Images", command=refresh_images).grid(
            row=3, column=0, sticky="ew", padx=8, pady=3
        )
        ttk.Button(frame, text="Ignore Selected", command=ignore_selected).grid(row=4, column=0, sticky="ew", padx=8, pady=3)
        ttk.Button(frame, text="Unignore Selected", command=unignore_selected).grid(
            row=5, column=0, sticky="ew", padx=8, pady=3
        )

        ttk.Separator(frame).grid(row=6, column=0, sticky="ew", padx=8, pady=10)
        ttk.Label(frame, text="Detector").grid(row=7, column=0, sticky="w", padx=8)
        ...
        self.detector_combo.grid(row=8, column=0, sticky="ew", padx=8, pady=3)
        self.detector_combo.bind("<<ComboboxSelected>>", self._on_detector_type_changed)

        ttk.Label(frame, text="Dictionary").grid(row=9, column=0, sticky="w", padx=8, pady=(6, 0))
        ...
        self.dictionary_combo.grid(row=10, column=0, sticky="ew", padx=8, pady=3)

        self.run_button = ttk.Button(frame, text="Run Detection On Non-Ignored Images", command=run_detection)
        self.run_button.grid(row=11, column=0, sticky="ew", padx=8, pady=(10, 3))
        ttk.Label(frame, textvariable=self.status_var, wraplength=310).grid(row=12, column=0, sticky="ew", padx=8, pady=8)
