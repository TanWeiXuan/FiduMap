...
@dataclass
class OmniRadTanCameraModel(CameraModel):
    """Unified omnidirectional (Mei) camera with radial-tangential distortion."""
    ...
    def project(self, ray: np.ndarray) -> np.ndarray:
        r = normalize_vectors(np.asarray(ray, dtype=float))
        if r.shape != (3,):
            raise ValueError(f"Expected ray shape (3,), got {r.shape}.")
        denom = r[2] + self.xi
        if denom <= 1e-12:
    ...
    def project_many(self, rays: np.ndarray) -> np.ndarray:
        ...
        if not np.any(mask):
            return out
        x = r[mask, 0] / denom[mask]
        y = r[mask, 1] / denom[mask]
        xd, yd = distort_radtan(x, y, self.k1, self.k2, self.p1, self.p2, self.k3)
    ...
    def unproject(self, pixel: np.ndarray) -> np.ndarray:
        ...
        r2 = x * x + y * y
        inside = 1.0 + (1.0 - self.xi * self.xi) * r2
        inside = max(inside, 0.0)
        alpha = (self.xi + np.sqrt(inside)) / (1.0 + r2)
        ray = np.array([alpha * x, alpha * y, alpha - self.xi], dtype=float)
        return normalize_vectors(ray)
    ...
    def unproject_many(self, pixels: np.ndarray) -> np.ndarray:
        ...
        alpha = (self.xi + np.sqrt(inside)) / (1.0 + r2)
        rays = np.column_stack((alpha * x, alpha * y, alpha - self.xi))
    ...
    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> "OmniRadTanCameraModel":
        return cls(**data)
    ...
    @classmethod
    def load_xml(cls, path: str | Path) -> "OmniRadTanCameraModel":
