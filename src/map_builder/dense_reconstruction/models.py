...
class FramePairRecord:
    ...
    optical_axis_angle_deg: float | None = None
    common_marker_count: int = 0
    estimated_overlap_score: float = 0.0
...
class PairMatchRecord:
    ...
    epipolar_error: float | None = None
    is_epipolar_inlier: int = 0
    is_used_for_track: int = 0
    id: int | None = None
...
class TrackRecord:
    ...
    min_triangulation_angle_deg: float | None = None
...
class PairSelectionConfig:
    min_baseline_m: float = 0.03
    max_baseline_m: float = 5.0
    max_optical_axis_angle_deg: float = 60.0
    min_common_markers: int = 0
    max_pairs_per_image: int = 10
    use_common_markers_bonus: bool = True
    use_projected_marker_overlap: bool = True


@dataclass
class MatchingConfig:
    max_pairs_to_match: int | None = None
...
class EpipolarFilterConfig:
    error_type: str = "ray_angular"
    max_ray_angular_error: float = 0.003
    max_sampson_error: float = 1e-4
    min_baseline_m: float = 0.03
    min_inliers: int = 8
    force_recompute: bool = False
...
class DuplicateMergeConfig:
    duplicate_merge_radius_m: float = 0.02
    max_merged_mean_reprojection_error_px: float = 3.0
    max_merged_reprojection_error_px: float = 6.0


@dataclass
class DenseBAConfig:
    mode: str = "points_only"
...
class DenseReconstructionConfig:
    ...
    epipolar: EpipolarFilterConfig = field(default_factory=EpipolarFilterConfig)
    triangulation: TriangulationConfig = field(default_factory=TriangulationConfig)
    duplicate_merge: DuplicateMergeConfig = field(default_factory=DuplicateMergeConfig)
    dense_ba: DenseBAConfig = field(default_factory=DenseBAConfig)
