...
from .backends import DepthAnythingV2AlignedBackend
from .export import export_stored_artifact
from .models import BACKEND_DAV2, MetricDepthConfig, MetricDepthProgress, MetricDepthRunSummary
from .anchor_builder import build_alignment_anchors
...
class MetricDepthPipeline:
    def __init__(self, project_folder: Path, backend_factory: Callable[[], Any] | None = None):
        ...
        self.backend_factory = backend_factory or DepthAnythingV2AlignedBackend
    ...
    def _run_images(self, image_ids: list[int], config: MetricDepthConfig, progress: Progress | None, cancel_event: Any) -> MetricDepthRunSummary:
        ...
        run_id = self.store.create_run(BACKEND_DAV2, config.model_id_or_path, config.to_dict(), ba_run_id, signature)
        ...
        try:
            ...
            self._emit(progress, "loading_model", "Loading Depth Anything V2 Small model", 0, len(image_ids), None, 0.0)
            ...
            message = f"{BACKEND_DAV2}; loading_model; missing model or optional dependency: {exc}"
        ...
        for index, image_id in enumerate(image_ids, 1):
            if _cancelled(cancel_event):
                summary.cancelled = True
                break
            if not config.recompute and self.store.latest_successful_record(image_id, ba_run_id, BACKEND_DAV2) is not None:
                summary.skipped += 1
            ...
            try:
                ...
                self.store.record_failure(run_id, image_id, stage, f"{BACKEND_DAV2}; image {image_id}; {exc}", getattr(exc, "metrics", None))
                summary.failed += 1
                LOG.warning("Metric depth failed for backend=%s image=%s stage=%s: %s", BACKEND_DAV2, image_id, stage, exc)
    ...
    def _process_one(self, image_id: int, config: MetricDepthConfig, ba_run_id: int, dense_signature: str, index: int, total: int, progress: Progress | None, cancel_event: Any):
        ...
        artifact.backend = BACKEND_DAV2
        artifact.metadata.update({
            "image_id": image_id, "image_relative_path": record.rel_path, "backend": BACKEND_DAV2,
        ...
        self._emit(progress, stage, f"Image {index}/{total} — aligned against metric anchors", index, total, image_id, (index - 0.25) / total)
    ...
    def _require_dav2_run(self, run_id: int) -> None:
        run = self.store.get_run(run_id)
        if run is None or str(run["backend"]) != BACKEND_DAV2:
            raise RuntimeError(f"Run {run_id} is not a Depth Anything V2 aligned run.")
