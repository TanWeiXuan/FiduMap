...
def _dense_panel(root):
    return DenseControlPanel(
        root,
...
def test_dense_count_text_and_run_all_control(tk_root):
    panel = _dense_panel(tk_root)
    panel.set_counts(
        {
            "feature_images": 2,
            "keypoints": 3500,
...
def test_dense_panel_rejects_invalid_numeric_settings(tk_root):
    panel = _dense_panel(tk_root)
